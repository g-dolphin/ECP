...
# now the aggregation
adf_byc <- adf %>% group_by(year,country,gloria_sector,gloria_sector_name) %>%
...
ggsave(gg,filename=paste0("euetsalloc_obaproxy",".svg"),
       device="svg",
       height=10,
       width=20,
       path=wd)
...
write.csv(adf_byc,file.path(fpr,"euetspermits_gloria_byc.csv"),row.names=F)
